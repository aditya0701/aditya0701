...
A fully reproducible deep-learning pipeline that finds and measures synaptic boutons buried in 3D fluorescent confocal microscopy scans of the *Drosophila* brain. I fine-tuned and benchmarked **four architectures head-to-head on custom data**: **MicroSAM** and **CellPoseSAM-3D** — both Segment Anything foundation models built on **Vision Transformer** encoders — the **SwinUNETR 3D** transformer, and the **nnU-Net v2** CNN, all trained across multiple preprocessing variants and evaluated with physical-volume-aware instance metrics. The fine-tuned **MicroSAM** model was the state-of-the-art winner, and is the model this thesis proposes.
